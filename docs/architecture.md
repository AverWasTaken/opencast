...
Future work: filesystem notifications with full-scan recovery, single-instance activation and resident global hotkey, code signing and automatic updates, accessible result-row widgets, and packaging smoke tests on a real Windows desktop. Keep extension execution separate from the trusted search/calculator core when that feature is introduced.
